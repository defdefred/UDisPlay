...
int UDisPlayBufferSize;

#define UDisPlayFill( T ) UDisPlayBufferSize=snprintf( &UDisPlay[0], UDisPlay_MAX, T )

void UDisPlayStart( char *ip, uint16_t port) {
  UDisPlay_UDP.beginPacket( ip, port );
  UDisPlayFill( "Hello world" );
  UDisPlay_UDP.write( (uint8_t*)&UDisPlay[0], UDisPlayBufferSize );
